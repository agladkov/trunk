...
class PGQueue(object):
    ...
    def get(self, name, block=True, timeout=None):
        try:
            channel, payload = self.trunk.get(
                name, block=block, timeout=timeout)
        except Empty:
            pass
        with self.trunk.cursor() as cursor:
            cursor.execute(
                "SELECT id, message FROM public.pop_lock(%s)", (name,))
            row = cursor.fetchone()
            if row is None:
                raise Empty()
            return row
    ...
    def put(self, name, message):
        with self.trunk.cursor() as cursor:
            cursor.execute(
                "INSERT INTO public.trunk_queue "
                "(name, message) VALUES (%s, %s)",
                (name, Json(message)))
    ...
    def purge(self, name):
        ...
        with self.trunk.cursor() as cursor:
            cursor.execute(
                "DELETE FROM public.trunk_queue WHERE name = %s",
                (name,))
        return size
